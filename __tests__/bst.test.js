...
describe('binarySearchTree', () => {
 ...
 test('it should return false if the node is the root ', () => {
   let node = new BSTNode(4)
   expect(pbst.remove(node)).toEqual(true);
  });
  test('it should return pbst.root to true if the node to be removed is the root', () => {
    let node = new BSTNode(4)
    expect(pbst.remove(node)).toEqual(true);
  });
  test('it should return rbst.root to false if it doesnt have any childs', () => {
    rbst.insert(new BSTNode(5));
    let node = new BSTNode(4)
    expect(rbst.remove(node)).toEqual(false);
  });
  
  test('it should return false if the node does not exist', () => {
    let node = new BSTNode(17)
    expect(pbst.remove(node)).toEqual(false);
  });
  test('it should return true if the node exists', () => {
    let node = new BSTNode(2)
    expect(pbst.remove(node)).toEqual(true);
  });
  test('it should return true if the node is a leaf node', () => {
    let node = new BSTNode(7)
    expect(pbst.remove(node)).toEqual('true');
  });
  test('it should return true if the node is a leaf node', () => {
    expect(pbst.root.right.right).toEqual(null);
});
});
